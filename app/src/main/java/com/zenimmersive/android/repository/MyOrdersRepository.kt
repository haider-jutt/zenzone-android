...
class MyOrdersRepository(context: Context) : BaseRepository(context) {
    val apiInterface: ServerApi = getApiServer()

    suspend fun getMyOrders(resModel : OrderListResModel): Resource<OrderListResModel> {
        ...
        val call: Call<ResponseBody> = apiInterface.getOrdersList(userId, userToken)
