...
class FavoritesRepository(context: Context) : BaseRepository(context) {
    val apiInterface: ServerApi = getApiServer()

    suspend fun getFavoriteSongs(resModel : FavoritesResModel): Resource<FavoritesResModel> {
        ...
        val call: Call<ResponseBody> = apiInterface.getFavoritesSongs(userId, userToken)
