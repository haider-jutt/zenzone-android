...
class DashboardRepository(context: Context) : BaseRepository(context) {
    val apiInterface: ServerApi = getApiServer()

    suspend fun updateDeviceToken(deviceToken: String, resModel : BaseResponse): Resource<BaseResponse> {
        ...
        val call: Call<ResponseBody> = apiInterface.deviceTokenUpdateApi(userId,deviceToken)
