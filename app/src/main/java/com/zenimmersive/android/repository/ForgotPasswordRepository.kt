...
class ForgotPasswordRepository(context: Context) : BaseRepository(context) {
    ...
    suspend fun forgotPassword(email: String, resModel : UserBasicResModel): Resource<UserBasicResModel> {
        val call: Call<ResponseBody> = apiInterface.forgotPassword(email, KeyStorage.getInstance(context).getString(APP_SELECTED_LANGUAGE).ifEmpty { "en" })
