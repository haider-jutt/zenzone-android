...
class VipMusicRepository(context: Context) : BaseRepository(context) {
    val apiInterface: ServerApi = getApiServer()

    suspend fun getVipMusics(resModel : VipSongsResModel): Resource<VipSongsResModel> {
        ...
        val call: Call<ResponseBody> = apiInterface.getVipSongs(userId, userToken)
