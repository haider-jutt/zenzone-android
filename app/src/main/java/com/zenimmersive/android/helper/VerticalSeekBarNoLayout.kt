 ...
 */
open class VerticalSeekBarNoLayout : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            val attributes =
                context.obtainStyledAttributes(attrs, R.styleable.VerticalSeekBarNoLayout, 0, 0)
            cornerRadius = attributes.getDimensionPixelSize(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_radius,
                15
            ).toFloat()

            startColor = attributes.getColor(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_start_color,
                Color.parseColor(DEFAULT_DRAWABLE_PROGRESS_START)
            )
            endColor = attributes.getColor(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_end_color,
                Color.parseColor(DEFAULT_DRAWABLE_PROGRESS_END)
            )
            maxValue = attributes.getInteger(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_max_value,
                DEFAULT_MAX_VALUE
            )
            progress = attributes.getInteger(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_progress,
                DEFAULT_PROGRESS
            )

            textSize = attributes.getDimensionPixelSize(
                R.styleable.VerticalSeekBarNoLayout_vsbnl_text_size,
                50
            ).toFloat()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Draw the progress
        val progressHeight = height * (progress / maxValue.toFloat())
        val progressRectF = RectF(0f, height - progressHeight, width.toFloat(), height.toFloat())
        ...
        canvas.drawText("${progress}%", width.div(2f), height.div(2f), textPaint)
    }
    ...
    private fun updateProgress(y: Float) {
        val newProgress = maxValue - (maxValue * (y / height)).toInt()
        progress = when {
            newProgress < 0 -> 0
